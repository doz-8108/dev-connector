...
router.post(
	...
	async (req, res) => {
		const errors = validationResult(req); // Get the validated result

		if (!errors.isEmpty()) {
			// If validation failed #return is crucial!
		...
		try {
			...
			const payload = {
				// !!mongoose help us to get rid of the underscore of id
				user: {
					id: user.id
				}
			};
			...
			jwt.sign(
				payload, // Generate the key jwt.sign(payload, secretkey, options, callback);
