...
router.post(
	"/",
	[auth, [check("text", "Content is required!").notEmpty()]],
	async (req, res) => {
		...
		try {
			...
			res.json(await postModel.find().sort({ date: -1 }));
		} catch (err) {
...
router.delete("/:post_id", auth, async (req, res) => {
	try {
		...
		await post.remove();
		res.send(await postModel.find().sort({ date: -1 }));
	} catch (err) {
		if (err.kind === "ObjectId") {
			return res.status(404).send("Post not found");
		}
