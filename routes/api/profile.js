...
// Get a profile for current logged in user
router.get("/me", auth, async (req, res) => {
	try {
		const foundUser = await profileModel
...
router.get("/", async (req, res) => {
	try {
		const foundUsers = await profileModel
			.find()
			.populate("user", ["name", "avatar"]); // return an array of all users

		res.json(foundUsers); // array of object
	} catch (error) {
		res.status(500).send("Server Error");
	}
});
...
router.get("/user/:user_id", async (req, res) => {
	try {
		const foundProfile = await profileModel
...
router.get("/github/:username", async (req, res) => {
	try {
		...
		const { status, data } = await axios.get(url, {
			headers: { Authorization: `token ${config.get("githubtoken")}` }
		});
